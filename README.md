...
処理は段階ごとに `output/` へ保存されます。中断した場合や、パラメータを一部だけ変えて再実行する場合は `--resume` を付けると、再計算が必要なステージだけが実行されます(例: `--goal` だけ変えた場合、文字起こしは再利用され採点からやり直し)。
...
V1は音声ベースの解析のみです。将来的に映像フレーム解析(Vision)を追加できるよう、採点モジュールは独立させてあります。
